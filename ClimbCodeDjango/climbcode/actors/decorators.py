...
def user_school_license_active(function):
    """Decorador que verifica que el usuario Alumno/Profesor pertenezca a una escuela con licencia activa"""

    def wrap(request, *args, **kwargs):

        # Si el usuario es alumno
        if hasattr(request.user.actor, 'teacher'):
            school = request.user.actor.teacher.school_t

        elif hasattr(request.user.actor, 'student'):
            school = request.user.actor.student.school_s
